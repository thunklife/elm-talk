...
Elm.Main.make = function (_elm) {
   ...
   var drawShip = F2(function (gameHeight,
   ship) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: $Basics.toFloat(ship.posX)
                                    ,_1: $Basics.toFloat(ship.posY)})($Graphics$Collage.rotate($Basics.degrees(90))($Graphics$Collage.filled($Color.blue)($Graphics$Collage.circle(20))));
   });
   var drawGame = F2(function (w,
   h) {
      return $Graphics$Collage.filled($Color.gray)(A2($Graphics$Collage.rect,
      w,
      h));
   });
   ...
   var boost = 5;
   var update = F2(function (point,
   ship) {
      return _U.replace([["posX"
                         ,ship.posX + point.x * boost]
                        ,["posY"
                         ,ship.posY + point.y * boost]],
      ship);
   });
   var initialShip = {_: {}
                     ,posX: 0
